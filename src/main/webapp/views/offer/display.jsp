...
<fieldset>
    ...
    <acme:labelvalue code="offer.commercial" href="actor/display.do?actorId=${offer.commercial.id}"
                     value="${offer.commercial.name} ${offer.commercial.surname}"/>
    ...
    <jstl:if test="${offer.commercial.id == actor.id}">
        ...
        <jstl:if test="${offer.inDevelopment}">
            <acme:button text="offer.createCollaborationRequest"
                         url="collaborationRequest/commercial/create.do?offerId=${offer.id}" css="formButton toLeft"/>
            <acme:button text="offer.createAdministrativeRequest"
                         url="administrativeRequest/create.do?offerId=${offer.id}" css="formButton toLeft"/>
        </jstl:if>
    </jstl:if>

    <security:authorize access="hasRole('EXECUTIVE')">
        <jstl:if test="${offer.inDevelopment}">
            <acme:button text="offer.deny" url="offer/executive/edit.do?offerId=${offer.id}"
                         css="formButton toLeft red"/>
        </jstl:if>
    </security:authorize>
    ...
    <jstl:if test="${offer.commercial.id != actor.id}">
        ...
        <jstl:if test="${!offer.published}">
            <security:authorize access="hasRole('COMMERCIAL')">
                <acme:button url="offer/commercial/listOffersByCollaboration.do" text="offer.back"
                             css="formButton toLeft"/>
            </security:authorize>
            <security:authorize access="hasRole('ADMINISTRATIVE')">
                <acme:button url="offer/administrative/listOffersByCollaboration.do" text="offer.back"
                             css="formButton toLeft"/>
            </security:authorize>
            <security:authorize access="hasRole('EXECUTIVE')">
...
<security:authorize access="hasRole('COMMERCIAL')">
    <jstl:if test="${offer.inDevelopment && offer.commercial.id == actor.id}">
        <acme:button text="subSection.create" url="subSection/commercial/create.do?offerId=${offer.id}"
                     css="formButton toLeft"/>
    </jstl:if>
</security:authorize>
