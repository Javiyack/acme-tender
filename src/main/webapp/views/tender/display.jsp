...
<fieldset>
    ...
    <jstl:if test="${tender.administrative.id == actor.id}">
        <acme:button url="tender/administrative/edit.do?tenderId=${tender.id}" text="tender.edit"
...
<fieldset>
    ...
    <jstl:if test="${tender.offer != null && tender.offer.published}">
        ...
        <acme:labelvalue code="offer.commercial" href="actor/display.do?actorId=${tender.offer.commercial.id}"
                         value="${tender.offer.commercial.name} ${tender.offer.commercial.surname}"/>
        <acme:labelvalue code="offer.comision" value="${tender.offer.amount * (benefitsPercentage/100)}"
                         isCurrency="true"/>
    ...
    <jstl:if test="${tender.offer == null}">
        <jstl:if test="${tender.offertable}">
            <security:authorize access="hasRole('COMMERCIAL')">
                <acme:button url="offer/commercial/create.do?tenderId=${tender.id}" text="tender.create.offer"
                             css="formButton toLeft"/>
            </security:authorize>
...
<fieldset>
    ...
    <jstl:if test="${tender.administrative.id == actor.id}">
        <acme:button text="tender.evaluationCriteria.create"
                     url="evaluationCriteria/administrative/create.do?tenderId=${tender.id}" css="formButton toLeft"/>
    </jstl:if>
</fieldset>
...
<fieldset>
    ...
    <security:authorize access="hasRole('COMMERCIAL')">
        <acme:button text="tender.comment.create" url="comment/commercial/create.do?tenderId=${tender.id}"
                     css="formButton toLeft"/>
    </security:authorize>
