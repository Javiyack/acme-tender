...
<display:table pagesize="${pageSize}" class="displaytag" name="tenders" requestURI="${requestUri}" id="row">
    ...
    <acme:column property="maxPresentationDate" title="tender.maxPresentationDate" css="${rowCss}"
                 format="display.date.time.format" sortable="true"/>

    <spring:message code="tender.comision" var="titleTenderComision"/>
    <display:column title="${titleTenderComision}" class="${rowCss}">
        <fmt:formatNumber value="${row.estimatedAmount * (benefitsPercentaje/100)}" pattern="${formpriceformat}"
                          var="varTenderComision"/>
        <div>
