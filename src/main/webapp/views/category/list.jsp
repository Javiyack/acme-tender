...
<fieldset>
    ...
    <display:table pagesize="5" class="displaytag" keepStatus="true" name="categories" requestURI="${requestUri}"
                   id="row">
        ...
        <display:column>
            <a href="category/administrator/list.do?parentCategoryId=${row.id}"> <spring:message
...
<jstl:if test="${parent != null}">
    <acme:button url="category/administrator/create.do?parentCategoryId=${parent.id}" text="category.create"
...
<jstl:if test="${parent != null && parent.fatherCategory != null}">
    <acme:button url="category/administrator/list.do?parentCategoryId=${parent.fatherCategory.id}"
                 text="category.move.to.up.level" css="formButton toLeft"/>
</jstl:if>
