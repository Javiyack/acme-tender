...
<display:table pagesize="5" class="displaytag" name="administrativeRequests" requestURI="${requestURI }" id="row">
    ...
    <jstl:choose>
        <jstl:when test="${requestURI == 'administrativeRequest/listSent.do' }">
            ...
            <display:column value="${row.administrative.name } ${row.administrative.surname }" title="${receiverVar }"
                            sortable="true"/>
        </jstl:when>
        <jstl:otherwise>
            <spring:message code="administrativeRequest.sender" var="senderVar"/>
            <display:column value="${row.offer.commercial.name } ${row.offer.commercial.surname }" title="${senderVar }"
                            sortable="true"/>
        </jstl:otherwise>
    </jstl:choose>
