...
<fieldset>
    ...
    <acme:labelvalue code="administrativeRequest.tender.reference"
                     value="${administrativeRequest.offer.tender.reference}"/>
...
<fieldset>
    ...
    <jstl:choose>
        <jstl:when test="${principal eq administrativeRequest.offer.commercial }">
            <acme:labelvalue code="administrativeRequest.receiver"
                             value="${administrativeRequest.administrative.name} ${administrativeRequest.administrative.surname} "/>
            <br/>
        </jstl:when>
        <jstl:when test="${principal eq administrativeRequest.administrative }">
            <acme:labelvalue code="administrativeRequest.sender"
                             value="${administrativeRequest.offer.commercial.name} ${administrativeRequest.offer.commercial.surname}"/>
            <br/>
        </jstl:when>
    </jstl:choose>
    ...
    <acme:labelvalue code="administrativeRequest.subSectionDescription"
                     value="${administrativeRequest.subSectionDescription}"/>

    <spring:message code="date.pattern" var="dateFormat"/>
    <fmt:formatDate value="${administrativeRequest.maxAcceptanceDate}" pattern="${dateFormat}"
...
<jstl:choose>
    ...
    <jstl:when test="${principal eq administrativeRequest.administrative }">
        <acme:cancel url="administrativeRequest/listReceived.do" code="administrativeRequest.back"
...
<jstl:if
        test="${principal eq administrativeRequest.administrative && administrativeRequest.accepted == null && !administrativeRequest.offer.published && administrativeRequest.answerable}">
    <acme:button url="administrativeRequest/reject.do?requestId=${administrativeRequest.id }"
                 text="administrativeRequest.reject" css="formButton toLeft"/>
    <acme:button url="administrativeRequest/accept.do?requestId=${administrativeRequest.id }"
                 text="administrativeRequest.accept" css="formButton toLeft"/>
</jstl:if>
