    ...
    <jstl:choose>
        <jstl:when test="${ editing eq false}">
            <input type="button" name="back"
                   ...
                   onclick="javascript:relativeRedir('folder/display.do?folderId=${parentFolder.id}')"
                   class="formButton toLeft"/>
        </jstl:when>
        <jstl:otherwise>
            <input type="button" name="back"
                   value="<spring:message code="folder.back"/>"
                   onclick="javascript:relativeRedir('folder/display.do?folderId=<jstl:out
                           value="${folder.getId()}"/>')" class="formButton toLeft"/>
        </jstl:otherwise>
