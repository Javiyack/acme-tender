...
<fieldset>
    ...
    <acme:labelvalue code="collaborationRequest.tender.reference"
                     value="${collaborationRequest.offer.tender.reference}"/>
...
<fieldset>
    ...
    <jstl:choose>
        <jstl:when test="${principal eq collaborationRequest.offer.commercial }">
            <acme:labelvalue code="collaborationRequest.receiver"
                             value="${collaborationRequest.commercial.name} ${collaborationRequest.commercial.surname} "/>
        </jstl:when>
        <jstl:when test="${principal eq collaborationRequest.commercial }">
            <acme:labelvalue code="collaborationRequest.sender"
                             value="${collaborationRequest.offer.commercial.name} ${collaborationRequest.offer.commercial.surname}"/>
        </jstl:when>
    </jstl:choose>
    ...
    <acme:labelvalue code="collaborationRequest.subSectionDescription"
                     value="${collaborationRequest.subSectionDescription}"/>
    <acme:labelvalue code="collaborationRequest.benefitsPercentage"
                     value="${collaborationRequest.benefitsPercentage} "/>
    <acme:labelvalue code="collaborationRequest.benefits.amount"
                     value="${(collaborationRequest.benefitsPercentage/100) * (benefitsPercentage/100) * collaborationRequest.offer.amount} "
                     isCurrency="true"/>
    ...
    <fmt:formatDate value="${collaborationRequest.maxAcceptanceDate}" pattern="${dateFormat}"
                    var="maxAcceptanceDateVar"/>
...
<jstl:choose>
    <jstl:when test="${principal eq collaborationRequest.offer.commercial }">
        <acme:cancel url="collaborationRequest/commercial/listSent.do" code="collaborationRequest.back"
                     css="formButton toLeft"/>
    </jstl:when>
    <jstl:when test="${principal eq collaborationRequest.commercial }">
        <acme:cancel url="collaborationRequest/commercial/listReceived.do" code="collaborationRequest.back"
                     css="formButton toLeft"/>
    </jstl:when>
</jstl:choose>
...
<jstl:if
        test="${principal eq collaborationRequest.commercial && collaborationRequest.accepted == null  && !collaborationRequest.offer.published && collaborationRequest.answerable}">
    <acme:button url="collaborationRequest/commercial/reject.do?requestId=${collaborationRequest.id }"
                 text="collaborationRequest.reject" css="formButton toLeft"/>
    <acme:button url="collaborationRequest/commercial/accept.do?requestId=${collaborationRequest.id }"
                 text="collaborationRequest.accept" css="formButton toLeft"/>
</jstl:if>
